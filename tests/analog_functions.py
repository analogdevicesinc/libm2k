...
def test_dual_channel_sync(
    ...
    Validation Strategy:
        ...
        5. Enforces the period ratio: exactly 5 triangle periods must occur between
           two consecutive stair-step falling edges. The ramp resets (5 -> 1) every
           5 stair steps, and each step lasts as long as one triangle period, so a
           correctly synchronized start yields exactly 5 triangle maxima per stair
           cycle. An out-of-sync start shows up as 6 (or 4) periods per cycle even
           when the per-edge alignment check still passes.
    """
    ...
    ch1_buffer = np.tile(np.arange(1,6), 4).astype(float)
    ...
    # Enforce the period ratio: exactly 5 triangle periods before the first stair
    # falling edge. Each stair step lasts one triangle period and the ramp resets
    # (5 -> 1) after 5 steps, so a correctly synchronized start yields exactly 5
    # completed triangle periods by the time the first falling edge appears. An
    # out-of-sync start shows up as 6 (or 4) periods here even when the per-edge
    # minima alignment above still passes.
    # NOTE: A full stair cycle is 5 * (n_samples / dac_sr) = 50 ms, but the capture
    # window (in_samples / adc_sr) is only ~75 ms, so it contains a single falling
    # edge. We therefore validate the count *up to* the first edge rather than
    # between two consecutive edges.
    EXPECTED_PERIODS_BEFORE_FIRST_EDGE = 5

    # Expected triangle period in ADC samples, derived from hardware constants:
    #   period_seconds = (n_samples * ch0_oversampling) / dac_sr
    #   period_adc     = period_seconds * adc_sr
    expected_period = (n_samples * ch0_oversampling) * adc_sr / dac_sr
    # Peak-separation guard at ~60% of a period prevents a single flat-topped maximum
    # from being counted twice at this low ADC resolution while still resolving
    # adjacent periods.
    min_peak_distance = max(1, int(expected_period * 0.6))

    # Detect triangle peaks (maxima). One maximum marks one completed period.
    # Height gate at 50% of range rejects noise ripple and the flat idle region.
    peak_height = ch0_data.min() + 0.5 * ch0_range
    triangle_peaks, _ = find_peaks(
        ch0_data, height=peak_height, distance=min_peak_distance
    )

    first_edge = falling_edges[0]
    periods_before_first_edge = int(np.sum(triangle_peaks < first_edge))

    if periods_before_first_edge != EXPECTED_PERIODS_BEFORE_FIRST_EDGE:
        return False, (
            f"Period ratio error: {periods_before_first_edge} triangle periods before "
            f"the first stair falling edge (at sample {first_edge}), expected exactly "
            f"{EXPECTED_PERIODS_BEFORE_FIRST_EDGE} (out-of-sync start)"
        )

    return (
        True,
        f"Sync validated: {len(falling_edges)} stair step falling edge(s) detected, "
        f"all triangle minima within {tolerance} samples tolerance, and exactly "
        f"{EXPECTED_PERIODS_BEFORE_FIRST_EDGE} triangle periods before the first "
        f"falling edge",
    )