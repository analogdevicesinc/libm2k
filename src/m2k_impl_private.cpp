...
M2kImplPrivate::M2kImplPrivate(std::string uri, iio_context* ctx, std::string name, bool sync) :
	ContextImplPrivate(uri, ctx, name, sync),
	m_sync(sync)
{
	initialize();
	setTimeout(0);
...
void M2kImplPrivate::deinitialize()
{
...
void M2kImplPrivate::scanAllPowerSupply()
{
...
void M2kImplPrivate::scanAllDigital()
{
...
bool M2kImplPrivate::resetCalibration()
{
	return m_calibration->resetCalibration();
}

bool M2kImplPrivate::calibrateADC()
{
...
bool M2kImplPrivate::calibrateDAC()
{
...
double M2kImplPrivate::getAdcCalibrationGain(unsigned int chn)
{
	return m_calibration->getAdcGain(chn);
}

int M2kImplPrivate::getAdcCalibrationOffset(unsigned int chn)
{
	return m_calibration->getAdcOffset(chn);
}

double M2kImplPrivate::getDacCalibrationGain(unsigned int chn)
{
	return m_calibration->getDacGain(chn);
}

int M2kImplPrivate::getDacCalibrationOffset(unsigned int chn)
{
	return m_calibration->getDacOffset(chn);
}

void M2kImplPrivate::setAdcCalibrationGain(unsigned int chn, double gain)
{
...
void M2kImplPrivate::setAdcCalibrationOffset(unsigned int chn, int offset)
{
	if (chn >= getAnalogIn()->getNbChannels()) {
		THROW_M2K_EXCEPTION("No such ADC channel", libm2k::EXC_OUT_OF_RANGE);
	}
...
void M2kImplPrivate::setDacCalibrationOffset(unsigned int chn, int offset)
{
...
void M2kImplPrivate::setDacCalibrationGain(unsigned int chn, double gain)
{
	if (chn >= getAnalogOut()->getNbChannels()) {
		THROW_M2K_EXCEPTION("No such DAC channel", libm2k::EXC_OUT_OF_RANGE);
	}
...
M2kAnalogIn* M2kImplPrivate::getAnalogIn()
{
...
M2kAnalogIn* M2kImplPrivate::getAnalogIn(string dev_name)
{
...
M2kPowerSupply* M2kImplPrivate::getPowerSupply()
{
...
M2kDigital* M2kImplPrivate::getDigital()
{
...
M2kAnalogOut* M2kImplPrivate::getAnalogOut()
{
...
std::vector<M2kAnalogIn*> M2kImplPrivate::getAllAnalogIn()
{
	return m_instancesAnalogIn;
}

std::vector<M2kAnalogOut*> M2kImplPrivate::getAllAnalogOut()
{
	return m_instancesAnalogOut;
}

bool M2kImplPrivate::hasMixedSignal()
{
	auto digital_impl = dynamic_cast<M2kDigitalImpl*>(getDigital());
	return digital_impl->hasRateMux();
}

void M2kImplPrivate::startMixedSignalAcquisition(unsigned int nb_samples)
{
...
void M2kImplPrivate::stopMixedSignalAcquisition()
{
...
bool M2kImplPrivate::hasAnalogTrigger()
{
...
bool M2kImplPrivate::hasDigitalTrigger()
{
...
void M2kImplPrivate::initialize()
{
...
void M2kImplPrivate::blinkLed(const double duration, bool blocking)
{
	std::shared_ptr<DeviceGeneric> m_m2k_fabric = make_shared<DeviceGeneric>(m_context, "m2k-fabric");
...
void M2kImplPrivate::setLed(bool on)
{
	std::shared_ptr<DeviceGeneric> m_m2k_fabric = make_shared<DeviceGeneric>(m_context, "m2k-fabric");
	if (m_m2k_fabric->getChannel("voltage4", true)->hasAttribute("done_led_overwrite_powerdown")) {
...
string M2kImplPrivate::getFirmwareVersion()
{
	std::string tmp = ContextImplPrivate::getFirmwareVersion();
	if (tmp == "") {
...
bool M2kImplPrivate::hasContextCalibration()
{
	std::string values;
	double temperature;
...
std::map<double, std::shared_ptr<struct CALIBRATION_PARAMETERS>> &M2kImplPrivate::getLUT()
{
	return m_calibration_lut;
}

bool M2kImplPrivate::isCalibrated()
{
...
double M2kImplPrivate::getCalibrationTemperature(double temperature)
{
...
double M2kImplPrivate::calibrateFromContext()
{
