...
{
	std::string rev = "";
	const struct iio_attr *attr_hw_model = iio_context_find_attr(ctx, "hw_model");
	if (attr_hw_model !=  NULL) {
		const char *hw_rev_attr_val = iio_attr_get_static_value(attr_hw_model);
		if (hw_rev_attr_val) {
