...
#include "m2khardwaretrigger_fake_impl.hpp"
...
std::vector<std::string> M2kFakeHardwareTriggerImpl::m_trigger_analog_cond = {
	"edge-rising",
	"edge-falling",
	"level-low",
	"level-high",
...
std::vector<std::string> M2kFakeHardwareTriggerImpl::m_trigger_digital_cond = {
	"edge-rising",
	"edge-falling",
	"level-low",
	"level-high",
...
std::vector<std::string> M2kFakeHardwareTriggerImpl::m_trigger_mode = {
	"always",
	"analog",
	"digital",
	"digital_OR_analog",
...
std::vector<std::string> M2kFakeHardwareTriggerImpl::m_trigger_source = {
	"a",
	"b",
	"a_OR_b",
	"a_AND_b",
...
std::vector<std::string>M2kFakeHardwareTriggerImpl:: m_trigger_logic_mode = {
	"or",
	"and",
};
...
M2kFakeHardwareTriggerImpl::M2kFakeHardwareTriggerImpl(struct iio_context *ctx, bool init) :
	M2kHardwareTrigger()
{
	m_analog_trigger_device = make_shared<DeviceIn>(ctx, "m2k-adc-trigger");
	if (!m_analog_trigger_device) {
		THROW_M2K_EXCEPTION("No analog trigger available", libm2k::EXC_INVALID_PARAMETER);
	}
	...
	if (m_analog_channels.size() < 1) {
		THROW_M2K_EXCEPTION("Hardware trigger has no analog channels", libm2k::EXC_INVALID_PARAMETER);
	}

	if (m_digital_channels.size() < 1) {
		THROW_M2K_EXCEPTION("Hardware trigger has no digital channels", libm2k::EXC_INVALID_PARAMETER);
	}

	if (m_logic_channels.size() < 1) {
		THROW_M2K_EXCEPTION("Hardware trigger has no trigger_logic channels", libm2k::EXC_INVALID_PARAMETER);
	}

	if (!m_delay_trigger) {
		THROW_M2K_EXCEPTION("No delay trigger available", libm2k::EXC_INVALID_PARAMETER);
	}
	...
	if (!m_digital_trigger_device) {
		THROW_M2K_EXCEPTION("No digital trigger available", libm2k::EXC_INVALID_PARAMETER);
	}
}

M2kFakeHardwareTriggerImpl::~M2kFakeHardwareTriggerImpl()
{
...
void M2kFakeHardwareTriggerImpl::M2kFakeHardwareTriggerImpl::reset()
{
...
bool M2kFakeHardwareTriggerImpl::hasExternalTriggerOut() const
{
	return m_logic_channels.at(1)->hasAttribute("out_select");
}

bool M2kFakeHardwareTriggerImpl::hasExternalTriggerIn() const
{
	return false;
}

bool M2kFakeHardwareTriggerImpl::hasCrossInstrumentTrigger() const
{
	return false;
}


M2K_TRIGGER_CONDITION_DIGITAL M2kFakeHardwareTriggerImpl::getDigitalExternalCondition() const
{
	std::string buf = m_trigger_digital_cond[0];//m_digital_trigger_device->getStringValue(16, "trigger");
...
void M2kFakeHardwareTriggerImpl::setDigitalExternalCondition(M2K_TRIGGER_CONDITION_DIGITAL ext_cond)
{
	//m_digital_trigger_device->setStringValue(16, "trigger",
	//					 m_trigger_digital_cond[ext_cond]);
}

M2K_TRIGGER_CONDITION_DIGITAL M2kFakeHardwareTriggerImpl::getAnalogExternalCondition(unsigned int chnIdx)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}
	std::string buf = m_trigger_digital_cond[0];//m_digital_channels[chnIdx]->getStringValue("trigger");
...
void M2kFakeHardwareTriggerImpl::setAnalogExternalCondition(unsigned int chnIdx, M2K_TRIGGER_CONDITION_DIGITAL cond)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}
	...
	if (cond == NO_TRIGGER_DIGITAL) {
		THROW_M2K_EXCEPTION("Analog External condition: can't set NO_TRIGGER for this channel.", libm2k::EXC_INVALID_PARAMETER);
	}

	//m_digital_channels[chnIdx]->setStringValue("trigger", m_trigger_digital_cond[cond]);
}

M2K_TRIGGER_CONDITION_ANALOG M2kFakeHardwareTriggerImpl::getAnalogCondition(unsigned int chnIdx)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}

	std::string buf = m_trigger_analog_cond[0];//m_analog_channels[chnIdx]->getStringValue("trigger");
...
void M2kFakeHardwareTriggerImpl::setAnalogCondition(unsigned int chnIdx, M2K_TRIGGER_CONDITION_ANALOG cond)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}
...
M2K_TRIGGER_CONDITION_DIGITAL M2kFakeHardwareTriggerImpl::getDigitalCondition(DIO_CHANNEL chn)
{	
	std::vector<std::string> available_digital_conditions = getAvailableDigitalConditions();
	std::string trigger_val = available_digital_conditions[0];//m_digital_trigger_device->getStringValue(chn, "trigger", false);
	...
	if (it == available_digital_conditions.end()) {
		THROW_M2K_EXCEPTION("M2kDigital: Cannot read channel attribute: trigger", libm2k::EXC_INVALID_PARAMETER);
	}
...
M2K_TRIGGER_CONDITION_DIGITAL M2kFakeHardwareTriggerImpl::getDigitalCondition(unsigned int chn)
{
	DIO_CHANNEL idx = static_cast<DIO_CHANNEL>(chn);
	return getDigitalCondition(idx);
}

void M2kFakeHardwareTriggerImpl::setDigitalCondition(DIO_CHANNEL chn, M2K_TRIGGER_CONDITION_DIGITAL cond)
{
...
void M2kFakeHardwareTriggerImpl::setDigitalCondition(unsigned int chn, M2K_TRIGGER_CONDITION_DIGITAL cond)
{
	DIO_CHANNEL idx = static_cast<DIO_CHANNEL>(chn);
...
int M2kFakeHardwareTriggerImpl::getAnalogLevelRaw(unsigned int chnIdx)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}

	double val = 0;//m_analog_channels[chnIdx]->getDoubleValue("trigger_level");

	return static_cast<int>(val);
}

void M2kFakeHardwareTriggerImpl::setAnalogLevelRaw(unsigned int chnIdx, int level)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}
...
double M2kFakeHardwareTriggerImpl::getAnalogLevel(unsigned int chnIdx)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}
...
void M2kFakeHardwareTriggerImpl::setAnalogLevel(unsigned int chnIdx, double v_level)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}
...
double M2kFakeHardwareTriggerImpl::getAnalogHysteresis(unsigned int chnIdx)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}

	double hysteresis_raw = 0;//m_analog_channels[chnIdx]->getDoubleValue("trigger_hysteresis");
	return (hysteresis_raw * m_scaling.at(chnIdx));
}

void M2kFakeHardwareTriggerImpl::setAnalogHysteresis(unsigned int chnIdx, double hysteresis)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}
...
M2K_TRIGGER_MODE M2kFakeHardwareTriggerImpl::getAnalogMode(unsigned int chnIdx)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}

	std::string buf = m_trigger_mode[0];//m_logic_channels[chnIdx]->getStringValue("mode");
	auto it = std::find(m_trigger_mode.begin(),
			    m_trigger_mode.end(), buf.c_str());
	if  (it == m_trigger_mode.end()) {
		THROW_M2K_EXCEPTION("Unexpected value read from attribute: mode", libm2k::EXC_OUT_OF_RANGE);
	}

	return static_cast<M2K_TRIGGER_MODE>(it - m_trigger_mode.begin());
}

void M2kFakeHardwareTriggerImpl::setAnalogMode(unsigned int chnIdx, M2K_TRIGGER_MODE mode)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}
...
void M2kFakeHardwareTriggerImpl::setDigitalMode(DIO_TRIGGER_MODE trig_mode)
{
...
DIO_TRIGGER_MODE M2kFakeHardwareTriggerImpl::getDigitalMode()
{
	std::string trigger_mode = "";
	trigger_mode = m_trigger_logic_mode[0];//m_digital_trigger_device->getStringValue(DIO_CHANNEL_0,"trigger_logic_mode", false);

	auto it = std::find(m_trigger_logic_mode.begin(), m_trigger_logic_mode.end(),
			    trigger_mode.c_str());
	if (it == m_trigger_logic_mode.end()) {
		THROW_M2K_EXCEPTION("Cannot read channel attribute: trigger logic mode", libm2k::EXC_OUT_OF_RANGE);
	}
	return static_cast<DIO_TRIGGER_MODE>(it - m_trigger_logic_mode.begin());
}

M2K_TRIGGER_SOURCE_ANALOG M2kFakeHardwareTriggerImpl::getAnalogSource()
{
...
void M2kFakeHardwareTriggerImpl::setAnalogSource(M2K_TRIGGER_SOURCE_ANALOG src)
{
 ...
 * channel at a time.
 */
int M2kFakeHardwareTriggerImpl::getAnalogSourceChannel()
{
	int chnIdx = -1;
 ...
 */
void M2kFakeHardwareTriggerImpl::setAnalogSourceChannel(unsigned int chnIdx)
{
	if (chnIdx >= m_num_channels) {
		THROW_M2K_EXCEPTION("Channel index is out of range", libm2k::EXC_OUT_OF_RANGE);
	}
...
int M2kFakeHardwareTriggerImpl::getAnalogDelay() const
{
	double delay = 0;//m_delay_trigger->getDoubleValue("delay");
	return static_cast<int>(delay);
}

void M2kFakeHardwareTriggerImpl::setAnalogDelay(int delay)
{
	//m_delay_trigger->setLongValue("delay", delay);
}

int M2kFakeHardwareTriggerImpl::getDigitalDelay() const
{
	return (int)0;//m_digital_trigger_device->getLongValue(0, "trigger_delay", false);
}

void M2kFakeHardwareTriggerImpl::setDigitalDelay(int delay)
{
//	m_digital_trigger_device->setLongValue(0, delay, "trigger_delay", false);
}

void M2kFakeHardwareTriggerImpl::setDigitalStreamingFlag(bool val)
{
...
bool M2kFakeHardwareTriggerImpl::getDigitalStreamingFlag()
{
	return m_streaming_flag_digital;
}

void M2kFakeHardwareTriggerImpl::setAnalogStreamingFlag(bool val)
{
	/* Make sure the trigger is reset before enabling the streaming flag. */
	if (val) {
...
bool M2kFakeHardwareTriggerImpl::getAnalogStreamingFlag()
{
	return m_streaming_flag_analog;
}

std::vector<string> M2kFakeHardwareTriggerImpl::getAvailableDigitalConditions()
{
	return {"edge-rising",
		"edge-falling",
		"level-low",
...
struct SETTINGS* M2kFakeHardwareTriggerImpl::getCurrentHwSettings()
{
...
void M2kFakeHardwareTriggerImpl::setHwTriggerSettings(struct SETTINGS *settings)
{
	for (unsigned int i = 0; i < m_num_channels; i++) {
...
void M2kFakeHardwareTriggerImpl::setCalibParameters(unsigned int chnIdx, double scaling, double offset)
{
