...
M2kFakeCalibrationImpl::M2kFakeCalibrationImpl(struct iio_context* ctx, M2kAnalogIn* analogIn,
				       M2kAnalogOut* analogOut):
	m_cancel(false),
	m_ctx(ctx),
	m_m2k_adc(dynamic_cast<M2kFakeAnalogInImpl*>(analogIn)),
	m_m2k_dac(dynamic_cast<M2kFakeAnalogOutImpl*>(analogOut)),
...
M2kFakeCalibrationImpl::~M2kFakeCalibrationImpl()
{
}

bool M2kFakeCalibrationImpl::initialize()
{
	m_initialized = false;
...
bool M2kFakeCalibrationImpl::isInitialized() const
{
	return m_initialized;
}

void M2kFakeCalibrationImpl::setAdcInCalibMode()
{
...
void M2kFakeCalibrationImpl::setDacInCalibMode()
{
...
void M2kFakeCalibrationImpl::restoreAdcFromCalibMode()
{
...
void M2kFakeCalibrationImpl::restoreDacFromCalibMode()
{
...
void M2kFakeCalibrationImpl::configAdcSamplerate()
{
...
void M2kFakeCalibrationImpl::configDacSamplerate()
{
...
bool M2kFakeCalibrationImpl::calibrateADCoffset()
{
	double gain = 1.3;
	double range = 3.192;
...
bool M2kFakeCalibrationImpl::calibrateADCgain()
{
	int16_t tmp;
...
int M2kFakeCalibrationImpl::getAdcOffset(unsigned int channel)
{
...
double M2kFakeCalibrationImpl::getAdcGain(unsigned int channel)
{
	if (channel >= m_m2k_adc->getNbChannels()) {
		THROW_M2K_EXCEPTION("No such ADC channel", libm2k::EXC_OUT_OF_RANGE);
	}
...
void M2kFakeCalibrationImpl::setAdcOffset(unsigned int chn, int offset)
{
...
void M2kFakeCalibrationImpl::setAdcGain(unsigned int chn, double gain)
{
	if (chn >= m_m2k_adc->getNbChannels()) {
		THROW_M2K_EXCEPTION("No such ADC channel", libm2k::EXC_OUT_OF_RANGE);
	}
...
void M2kFakeCalibrationImpl::updateDacCorrections()
{
...
void M2kFakeCalibrationImpl::updateAdcCorrections()
{
	m_m2k_adc->setAdcCalibOffset(static_cast<ANALOG_IN_CHANNEL>(0), m_adc_ch0_offset, m_adc_ch0_vert_offset);
	m_m2k_adc->setAdcCalibOffset(static_cast<ANALOG_IN_CHANNEL>(1), m_adc_ch1_offset, m_adc_ch1_vert_offset);
...
bool M2kFakeCalibrationImpl::resetCalibration()
{
	if (!m_initialized) {
		initialize();
	}
...
bool M2kFakeCalibrationImpl::fine_tune(size_t span, int16_t centerVal0, int16_t centerVal1,
				   size_t num_samples)
{
...
int M2kFakeCalibrationImpl::getDacOffset(unsigned int channel)
{
	if (channel >= m_m2k_dac->getNbChannels()) {
		THROW_M2K_EXCEPTION("No such DAC channel", libm2k::EXC_OUT_OF_RANGE);
	}

	double offset = 2048;//m_ad5625_dev->getDoubleValue(channel, "raw", true);
	if (channel == 0) {
...
double M2kFakeCalibrationImpl::getDacGain(unsigned int channel)
{
	if (channel >= m_m2k_dac->getNbChannels()) {
		THROW_M2K_EXCEPTION("No such DAC channel", libm2k::EXC_OUT_OF_RANGE);
	}
...
void M2kFakeCalibrationImpl::setDacOffset(unsigned int chn, int offset)
{
...
void M2kFakeCalibrationImpl::setDacGain(unsigned int chn, double gain)
{
	if (chn >= m_m2k_dac->getNbChannels()) {
		THROW_M2K_EXCEPTION("No such DAC channel", libm2k::EXC_OUT_OF_RANGE);
	}
...
bool M2kFakeCalibrationImpl::calibrateDACoffset()
{
	int16_t tmp;
	bool calibrated = false;
	const unsigned int num_samples = 1e5;
...
bool M2kFakeCalibrationImpl::calibrateDACgain()
{
	int16_t tmp;
	bool calibrated = false;
	std::vector<std::vector<double>> ch_data = {};
...
bool M2kFakeCalibrationImpl::calibrateADC()
{
	bool ok;
	if (!m_initialized) {
		initialize();
...
bool M2kFakeCalibrationImpl::calibrateDAC()
{
	bool ok;
	if (!m_initialized) {
		initialize();
...
bool M2kFakeCalibrationImpl::calibrateAll()
{
	bool ok;
	initialize();
...
void M2kFakeCalibrationImpl::cancelCalibration()
{
	m_cancel=true;
}

bool M2kFakeCalibrationImpl::setCalibrationMode(int mode)
{
	std::string strMode;
	switch (mode) {
	case ADC_GND:
...
int16_t M2kFakeCalibrationImpl::processRawSample(int16_t value)
{
