...
ContextImplPrivate::ContextImplPrivate(std::string uri, struct iio_context *ctx, std::string name, bool sync)
{
	UNUSED(name);
	m_context = ctx;
	m_uri = uri;
...
ContextImplPrivate::~ContextImplPrivate()
{
...
void ContextImplPrivate::reset()
{
}

void ContextImplPrivate::deinitialize()
{
}


bool ContextImplPrivate::iioDevHasAttribute(iio_device* dev, std::string const& attr)
{
...
bool ContextImplPrivate::iioDevBufferHasAttribute(iio_device *dev, const std::string &attr)
{
	const char *attribute = iio_device_find_buffer_attr(dev, attr.c_str());
	return attribute != nullptr;
}

bool ContextImplPrivate::iioChannelHasAttribute(iio_channel* chn, std::string const& attr)
{
...
DEVICE_DIRECTION ContextImplPrivate::getIioDeviceDirection(std::string dev_name)
{
...
DEVICE_TYPE ContextImplPrivate::getIioDeviceType(std::string dev_name)
{
	auto dev = iio_context_find_device(m_context, dev_name.c_str());
	if (!dev) {
		THROW_M2K_EXCEPTION("No device found with name: " + dev_name, libm2k::EXC_INVALID_PARAMETER);
...
std::vector<std::pair<std::string, std::string>> ContextImplPrivate::getIioDevByChannelAttrs(std::vector<std::string> attr_list)
{
...
std::vector<std::pair<std::string, std::string>> ContextImplPrivate::getHwmonDevices(){
	iio_device* dev = nullptr;
	iio_channel* chn = nullptr;
	std::vector<std::pair<std::string, std::string>> dev_chn_list;
	unsigned int nb_chn = 0;
...
bool ContextImplPrivate::isIioDeviceBufferCapable(std::string dev_name)
{
...
std::unordered_set<std::string> ContextImplPrivate::getAllDevices() const
{
	return Utils::getAllDevices(m_context);
}

void ContextImplPrivate::scanAllDMM()
{
...
void ContextImplPrivate::logAllAttributes() const
{
...
DMM* ContextImplPrivate::getDMM(std::string dev_name)
{
...
DMM* ContextImplPrivate::getDMM(unsigned int index)
{
...
std::vector<DMM*> ContextImplPrivate::getAllDmm()
{
	return m_instancesDMM;
}

unsigned int ContextImplPrivate::getDmmCount()
{
	return m_instancesDMM.size();
}


std::vector<std::string> ContextImplPrivate::getAvailableContextAttributes()
{
...
std::string ContextImplPrivate::getContextAttributeValue(std::string attr)
{
	std::string val;
...
std::string ContextImplPrivate::getContextDescription()
{
	if (!m_context) {
		return "";
	}
...
std::string ContextImplPrivate::getSerialNumber()
{
	return getContextAttributeValue("hw_serial");
}

M2k* ContextImplPrivate::toM2k()
{
...
Generic *ContextImplPrivate::toGeneric()
{
...
std::string ContextImplPrivate::getUri()
{
	return m_uri;
}

std::string ContextImplPrivate::getFirmwareVersion()
{
	return Utils::getFirmwareVersion(m_context);
}

void ContextImplPrivate::initializeContextAttributes()
{
	const char *name;
	const char *value;
	char ctx_git_tag[8];
...
const struct libm2k::IIO_CONTEXT_VERSION ContextImplPrivate::getIioContextVersion()
{
...
struct iio_context *ContextImplPrivate::getIioContext()
{
        return m_context;
}

void ContextImplPrivate::setTimeout(unsigned int timeout)
{
