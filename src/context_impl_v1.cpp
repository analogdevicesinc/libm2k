...
{
	struct iio_device *dev = iio_context_find_device(m_context, dev_name.c_str());
	if (dev == NULL) {
		return false;
	}
	unsigned int chn_nb = iio_device_get_channels_count(dev);
	for (unsigned int i = 0; i < chn_nb; i++) {
...
void ContextImpl::logAllAttributes() const
{
	...
	for (unsigned int i = 0; i < attr_no; i++) {
		...
		const struct iio_attr *attr = iio_context_get_attr(m_context, i);
		if (!attr) {
...
//		int ret = iio_context_get_attr(m_context, i, &name, &value);
		if (attr == NULL) {
			THROW_M2K_EXCEPTION("Device: Can't get context attribute " + std::to_string(i), libm2k::EXC_RUNTIME_ERROR);
		}
		pair.first = std::string(iio_attr_get_name(attr));
		pair.second = std::string(iio_attr_get_static_value(attr));
		LIBM2K_LOG(INFO, libm2k::buildLoggingMessage({"context", pair.first.c_str(), LIBM2K_ATTRIBUTE_READ}, pair.second.c_str()));
...
const struct libm2k::IIO_CONTEXT_VERSION ContextImpl::getIioContextVersion()
{
	...
	iioContextVersion.major = iio_context_get_version_major(m_context);
	iioContextVersion.minor = iio_context_get_version_minor(m_context);
